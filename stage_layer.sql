CREATE OR REPLACE TABLE `flight_to_2026.stg_airlines` AS (
  SELECT 
    UPPER(TRIM(Airline_Code)) AS airline_code, TRIM(Airline_Name) AS airline_name,
...
CREATE OR REPLACE TABLE flight_to_2026.stg_flight_schedule AS (
  SELECT 
    ...
    NULLIF(TRIM(CAST(Gate_ID AS STRING)), '') AS gate_id, 
    TRIM(Status) AS status, CURRENT_TIMESTAMP() AS load_timestamp
